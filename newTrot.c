...
void M_statechange()
{
	...
	else if(trotDirectionState==30)//高速绕杆
	{
		...
		rotate_in_place_by22_5_right(2);
		move_forward_for_steps(6);
		rotate_in_place_by22_5_left(6);
		move_forward_for_steps(11);
		rotate_in_place_by22_5_right(6);
		move_forward_for_steps(11);
		rotate_in_place_by22_5_left(6);
		trotDirectionState=0;
	}
	else if(trotDirectionState==34){//闭环绕杆（原地转＋直走）（真正的高速绕杆）
		rotate_in_place_to_targetAngle(-30.5);//角度越大稳定性越好
		move_forward_for_steps(5);//由于是5步而不是4步，起f始位置可偏后一些
		rotate_in_place_to_targetAngle(35.5);
		...
		rotate_in_place_to_targetAngle(-90);//角度越大稳定性越好
